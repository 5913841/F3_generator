...
#define FOR_EACH_NET_STATS(s, i) for (i = 0; (i < g_config->num_lcores) && ((s = g_net_stats_all[i]), 1); i++)
...
void net_stats_timer_handler()
{
    ...
    s->cpusage = cpuload_cal_cpusage(&g_config_percore->cpusage, time_in_config());
}

void net_stats_init()
{
    memset(&g_net_stats, 0, sizeof(struct net_stats));
    g_net_stats_all[g_config_percore->lcore_id] = &g_net_stats;
}
