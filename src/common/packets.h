...
inline void tx_queue_flush(struct tx_queue *tq, uint16_t port_id, uint16_t queue_id)
{
...
inline void send_packet_by_queue(struct tx_queue *tq, struct rte_mbuf *mbuf, uint16_t port_id, uint16_t queue_id)
{
    tq->tx[tq->tail] = mbuf;
    tq->tail++;
...
inline rte_mbuf *recv_packet_by_queue(struct rx_queue *rq, uint16_t port_id, uint16_t queue_id)
{

    if (rq->head == rq->tail)
    {
        rq->head = rq->tail = 0;
        int nb_rx = rte_eth_rx_burst(port_id, queue_id, rq->rx, rq->rx_burst);
        if (nb_rx == 0)
        {
            return nullptr;
        }
