...
public:
    Protocol(){};
    ProtocolCode name() { return PTC_NONE; }

    template<typename Socket>
    int construct(Socket* socket, rte_mbuf* data) { return 0; };

    template<typename Socket>
    size_t get_hdr_len(Socket* socket, rte_mbuf* data){ return 0; };
};
...
public:
    ...
    template<typename Socket>
    int send_frame(Socket* socket, rte_mbuf* data) { return 0; };
};
...
public:
    ...
    static PacketParser parser;
    template<typename Socket>
    int process(Socket* socket, rte_mbuf* data) { return 0; };
};
